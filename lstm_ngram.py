    ...
    def predict(self, text, tagger, upos_1h_labels):
        ...
        if len(upos_texts) == 3:
            ...
            if text in self.ngram_trainer.probabilities:
                ...
                possible_sentences = [text + ' ' + predicted_word for (predicted_word, predicted_probability) in ngram_predictions.items()]
                # Tokenise ngram possible sentences with stanfordnlp
                ngram_possible_sentences_doc = [tagger.tag(possible_sentence) for possible_sentence in possible_sentences]
                ...
                for i, ngram_possible_sentence_doc in enumerate(ngram_possible_sentences_doc):
                    ...
                    try:
                        ...
                        combined_predictions[ngram_prediction_word] = list(ngram_predictions.values())[i]*upos_confidence
                    except ValueError:
    ...
    def trial(self, filename):
        ...
        while True:
            ...
            if combined_predictions:
                for predicted_word, predicted_probability in sorted(combined_predictions.items(), key=itemgetter(1), reverse=True)[0:5]:
                    print(f"Predicted '{predicted_word}' with {predicted_probability*100}% confidence")

    def test(self, corpora):
        nlp = stanfordnlp.Pipeline()
        ngrams = self.ngram_trainer.generate_ngrams(corpora, self.n)
