...
def main():
    ...
    option: str = args.implementation if args.implementation else input("ngram/ngram with lstm/lstm with lstm: ")
    ngram_n_input: int = int(args.n) if args.n else int(input("ngram n: "))
    procedure: str = args.procedure if args.procedure else input("trial/test/train: ")
    corpora_path: str = args.corpora if args.corpora else input("corpora path: ")
    ...
    if procedure == "train":
        ...
        print(
            f"Implementation had {(correct / (correct + incorrect)) * 100}% accuracy on {correct + incorrect} iterations ({correct} correct, {incorrect} incorrect)")
